...
- **Robot et mises au point sont comptés à part** (`parOrigine`) : un appel lancé par GitHub Actions fait tourner le média, un appel lancé depuis un poste est un réglage. Les chiffres du haut de la page et le budget ne suivent que le robot ; la dépense de mise au point apparaît à côté, « hors budget ». Le tableau jour par jour, lui, additionne tout : c'est ce total que montre la console Anthropic.
