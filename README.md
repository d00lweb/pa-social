...
- **Le montant facturé fait foi** — `npm run couts:sync`. Le relevé maison est exact mais ne commence qu'au jour de sa mise en service : il ne peut pas répondre « combien ce mois-ci ». La commande interroge le rapport de coûts de l'organisation Anthropic et écrit `state/couts-console.json` ; `couts.html` l'affiche alors en tête (« Mois en cours — facturé »), et le budget comme les alertes Telegram se calent dessus. Entre deux synchronisations, c'est le plus élevé des deux qui compte : une alerte de budget doit se tromper du côté prudent.
  - Il faut une **clé d'administration** (console Anthropic → *Settings* → *API keys* → *Admin keys*), distincte de la clé du projet, en lecture seule sur la facturation. Elle va dans `.env` sous `ANTHROPIC_ADMIN_KEY` — **jamais** dans les secrets GitHub : une clé d'administration gouverne toute l'organisation, elle n'a rien à faire dans un dépôt public.
  - `state/couts-console.json` ne contient que des montants : le committer met la page à jour.
