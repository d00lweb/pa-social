...
| 18/09/2026 | **Localisation étendue.** Les zones d'identité employées comme rubriques (Périgord, Pays basque, Médoc, Île de Ré…) sont traduites en département avant la recherche d'identifiant, et la charte impose désormais le nom administratif dans `lieu.departement` ainsi que la commune dès qu'elle est nommée. Couverture réelle : toutes les villes via Wikidata, deux départements validés (Haute-Vienne, Deux-Sèvres). Les dix autres n'ont pas d'identifiant de lieu exploitable : Wikidata n'en connaît aucun, les pages Instagram de départements sont en ancien format refusé par Meta, la recherche de pages Facebook est fermée à notre application et celle de Threads exige `threads_location_tagging`. |
