...
| 25/09/2026 | **Le pont entre réseaux, qui rend la découverte utile sur Instagram.** Un pseudo fabriqué depuis le mot du domaine ne trouvera jamais l'Office français de la biodiversité, qui se nomme `@ofbiodiversite` et compte 50 383 abonnés — ni la Ligue pour la protection des oiseaux, qui est `@lpo_officiel` et non `@lpofrance` comme je l'avais supposé. Bluesky donne le **nom** de l'organisation, Wikidata et son site officiel donnent ensuite ses pseudos, `business_discovery` les vérifie. Deux vérifications enchaînées, aucune devinette. **Threads** est servi par le même chemin : un compte de référence trouvé sur Instagram y est mentionnable tel quel, après vérification de sa présence. |
| 25/09/2026 | **Mentions en fin de texte sur Bluesky et Threads.** Un compte de référence porte un nom de domaine, jamais écrit dans la phrase : la substitution ne peut pas l'atteindre. Il est donc ajouté en fin de post, sur sa propre ligne, un seul et seulement si la longueur le permet. **Kit X** : X n'ayant pas d'API gratuite, un message supplémentaire donne les organisations confirmées sur les autres réseaux pour cet article — le nom est sûr, le pseudo reste à trouver à la main. |
| 25/09/2026 | **Un nom d'un seul mot reste refusé, et la vérification dit pourquoi.** Pour un article sur l'escale de l'Hermione et du Belem à Bayonne, aucune des trois entités n'est taguée. Ce n'est pas un défaut : `@lebelem` est un **café bar festif** (644 abonnés), `@belem_officiel` une personne nommée Ali Belem (207), `@bayonne_officiel` un compte vide (502), et Wikidata renvoie pour « Belem » la mairie de Belém au Brésil. Assouplir la règle taguerait tout cela. Pour ces sujets-là, la table vérifiée `config/comptes.json` est la seule voie sûre. |
| 25/09/2026 | **Les homonymes de Wikidata départagés par le contexte de l'article.** `@troismatsbelem` et `@hermione_lafayette` existaient et n'étaient jamais trouvés. Deux causes, mesurées : la recherche n'examinait que **trois** fiches — celle du navire Hermione arrive en **huitième** position, derrière un prénom, un genre de plantes, Hermione Granger et un astéroïde — et elle retenait la première fiche renseignée, soit la **mairie de Belém au Brésil** pour « Belem ». La recherche examine désormais huit fiches et retient celle dont la description partage un mot avec l'article : « trois-mâts barque français », « navire de guerre français ». Son site officiel suffit alors — `fondationbelem.com` déclare `@troismatsbelem` et `@troismats_Belem`. Conséquence : **un nom d'un seul mot n'est plus écarté**, puisqu'il se résout sans aucune devinette — `variantesHandle` ne produit toujours rien pour un mot seul, et la recherche Bluesky lui reste fermée. L'Hermione, dont la fiche ne porte pas de site, est inscrite dans la table vérifiée. |
| 25/09/2026 | **L'échelle géographique : la commune avant le territoire élargi.** Plus l'audience est proche du sujet, plus elle a de raisons de suivre — qui suit `@bayonnemaville` lit un article sur une escale à Bayonne, qui suit un compte « Pays basque » n'y verra qu'une actualité parmi d'autres. Les comptes de la commune (ville et office de tourisme) sont donc cherchés et placés **avant** les viviers de territoire et de domaine, et la rotation joue à l'intérieur de chaque échelon, jamais entre eux. Les formes usuelles françaises — `ville{X}`, `{X}maville`, `visit{X}`, `{X}tourisme`, `mairie{X}` — sont stables : elles trouvent `@bayonnemaville` (54 116) et `@visitbayonne` (34 458), `@visitbordeaux` (118 223), `@limogestourisme` (22 539), `@daxtourisme` (5 295), `@montdemarsantourisme`. |
